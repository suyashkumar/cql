...
func (v *visitor) VisitQuantityContext(ctx cql.IQuantityContext) (model.Quantity, error) {
	...
	if valid, _ := ucum.ValidateUnit(unitStr, true, true); !valid {
		return model.Quantity{}, fmt.Errorf("internal error - invalid unit found when parsing quantity literal, got: %s", unitContext.GetText())
	}
...
func (v *visitor) VisitUnit(ctx cql.IUnitContext) (model.Unit, error) {
	...
	if ctx.STRING() != nil {
		...
		if valid, msg := ucum.ValidateUnit(unitStr, true, true); !valid {
			return model.UNSETUNIT, fmt.Errorf("internal error - invalid unit found when parsing quantity literal, got error: %s", msg)
		}
